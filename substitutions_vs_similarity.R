...
# for(i in 1:length(translations$Nucs)){
#   split.nucs <- unlist(strsplit(translations$Nucs[i],split = ""))
#   
#   
#   
# }
...
# for(i in 1:nrow(combined_all_data)){
#   
#   if(combined_all_data$AA_Ref[i] == combined_all_data$AA_Change[i]){
#     combined_all_data$distance[i] <- 0
#   }else{
#     
#     if(!is.na(miyata.table[translations$AA_short[which(translations$AA_letter == combined_all_data$AA_Change[i])[1]],
#                            translations$AA_short[which(translations$AA_letter == combined_all_data$AA_Ref[i])[1]]])){
#       combined_all_data$distance[i] <- miyata.table[translations$AA_short[which(translations$AA_letter == combined_all_data$AA_Change[i])[1]],
#                                                     translations$AA_short[which(translations$AA_letter == combined_all_data$AA_Ref[i])[1]]]
#     }else{
#       combined_all_data$distance[i] <- miyata.table[translations$AA_short[which(translations$AA_letter == combined_all_data$AA_Ref[i])[1]],
#                                                     translations$AA_short[which(translations$AA_letter == combined_all_data$AA_Change[i])[1]]]
#     }
#   }
# }


# save(combined_all_data,file="combined_data_with_distance.RData")
# 
# load("combined_data_with_distance.RData")
# 
# library(ggplot2)
# 
# frequency.plot <- ggplot(data = combined_all_data,aes(x=distance,y=freq)) + 
#   geom_point(alpha=.2) +  
...
#   theme_bw()
# 
# ggsave(filename = "prevalence_distance_plot.png",plot = frequency.plot)
# 
# library(tidyverse)
# library(reshape2)
# 
...
# colnames(miyata.melt) <- c("AA_Ref","AA_Change","Distance",unique(combined_all_data$tumor_type),"Between species")
# 
# miyata.melt <- melt(miyata.melt, id.vars = c("AA_Ref","AA_Change","Distance"))
# tail(miyata.melt)
# colnames(miyata.melt) <- c(colnames(miyata.melt)[1:3],"Tumor_type","Frequency")
# 
# length(which(!is.na(miyata.table)))
# length(unique(as.vector(miyata.table)))
# 
...
# }
# 
# save(miyata.melt,file = "miyata_melt.RData")
# 
# library(ggplot2)
# 
# frequency.plot <- ggplot(data = subset(miyata.melt, Frequency>0), aes(x=Distance, y= Frequency)) + geom_point() + geom_smooth(method="glm",method.args=list(family=gaussian(link="log")),se = F) + facet_wrap(~Tumor_type, scales = "free") + theme_classic() #+ scale_y_log10()
# 
# ggsave(filename = "Frequency_plot_all_added.png",plot = frequency.plot,height = 10,width = 10)
...
# load("~/Documents/Selection_analysis/LUAD/trinuc_output/trinuc_data_LUAD.RData")
# sum(trinuc.mutation_data$proportion)
# 
...
# transition.matrix[lower.tri(transition.matrix,diag=TRUE)] <- NA
# 
sum(transition.matrix[which(translations[,"AA_letter"]=="L"),which(translations[,"AA_letter"]=="L")],na.rm = T)
sum(transition.matrix[which(translations[,"AA_letter"]=="C"),which(translations[,"AA_letter"]=="P")])

# LEU  TRP
# THR  ILE
transition.matrix[which(translations[,"AA_letter"]=="I"),which(translations[,"AA_letter"]=="T")]
transition.matrix[which(translations[,"AA_letter"]=="C"),which(translations[,"AA_letter"]=="S")]

transition.matrix[which(translations[,"AA_letter"]=="S"),which(translations[,"AA_letter"]=="W")]
transition.matrix[which(translations[,"AA_letter"]=="E"),which(translations[,"AA_letter"]=="L")]
...
# View(combined_all_data[which(combined_all_data$tumor_type==miyata.melt$Tumor_type[i] & 
#                                combined_all_data$AA_Ref== translations$AA_letter[which(translations$AA_short== miyata.melt$AA_Change[i])[1]] & 
#                                combined_all_data$AA_Change== translations$AA_letter[which(translations$AA_short== miyata.melt$AA_Ref[i])[1]]),])
...
frequency.plot <- ggplot(data = subset(miyata.melt, Frequency>0), aes(x=Distance, y= proportion_observed/Potential_changes_proportion)) + geom_point(alpha=0.8) + geom_smooth(method="glm",method.args=list(family=gaussian(link="log")),se = F) + facet_wrap(~Tumor_type, scales = "free") + labs(y="Normalized frequency",x="Molecular distance") + theme_bw() 

ggsave(filename = "frequency_plot_relative.png",plot = frequency.plot,width = 10,height = 10)
...
ggplot(data = subset(miyata.melt, Tumor_type=="Between species" & Distance > 0),aes(x=Distance,y=relative_freq)) + geom_point() + coord_cartesian(ylim = c(0,6))



miyata.melt[which(miyata.melt$Tumor_type=="BLCA" & miyata.melt$relative_freq>5),]

# combined_all_data[which(combined_all_data$tumor_type=="BLCA" & combined_all_data$freq>2),][which.max(combined_all_data$gamma_epistasis[which(combined_all_data$tumor_type=="BLCA" & combined_all_data$freq>2)]),]
# 
# combined_all_data[which(combined_all_data$tumor_type=="BLCA" & combined_all_data$AA_Ref=="E" & combined_all_data$AA_Change=="K" & combined_all_data$freq>1),]
# 
# combined_all_data[which(combined_all_data$tumor_type=="BLCA" & combined_all_data$AA_Ref=="Q" & combined_all_data$AA_Change=="E" & combined_all_data$freq>1),]
# 
# 
# combined_all_data[which(combined_all_data$tumor_type=="BLCA" & combined_all_data$AA_Ref=="F" & combined_all_data$AA_Change=="F" & combined_all_data$freq>1),]
# 
# 
# 
# miyata.melt[which(miyata.melt$Tumor_type=="THCA" & miyata.melt$relative_freq>3.5),]
# 
# 
# combined_all_data[which(combined_all_data$tumor_type=="THCA" & combined_all_data$AA_Ref=="E" & combined_all_data$AA_Change=="V" & combined_all_data$freq>0),]
# 
# 
# combined_all_data[which(combined_all_data$tumor_type=="THCA" & combined_all_data$AA_Ref=="E" & combined_all_data$AA_Change=="K" & combined_all_data$freq>1),]
# 
# 
# 
# miyata.melt[which(miyata.melt$Tumor_type=="BRCA" & miyata.melt$relative_freq>4),]
# 
# combined_all_data[which(combined_all_data$tumor_type=="BRCA" & combined_all_data$AA_Ref=="E" & combined_all_data$AA_Change=="K" & combined_all_data$freq>1),]

library(ggrepel)
freq.plot.subset <- ggplot(data = subset(miyata.melt, Frequency>0 & Tumor_type %in% c("BLCA","BRCA","THCA")), aes(x=Distance, y= proportion_observed/Potential_changes_proportion)) + 
  geom_text_repel(data = subset(miyata.melt, Frequency>0 & Tumor_type %in% c("BLCA","BRCA") & AA_Ref=="GLU" & AA_Change=="LYS"),aes(x=Distance,y=relative_freq,label="PIK3CA E542K and E545K"),nudge_y = 1,nudge_x = 1) +
  geom_text_repel(data = subset(miyata.melt, Frequency>0 & Tumor_type %in% c("THCA") & AA_Ref=="GLU" & AA_Change=="VAL"),aes(x=Distance,y=relative_freq,label="BRAF V600E"),nudge_y = 1,nudge_x = 1) +
  geom_point(alpha=0.8) + 
  geom_smooth(method="glm",method.args=list(family=gaussian(link="log"))) + 
  facet_wrap(~Tumor_type, scales = "free") + labs(y="Normalized frequency",x="Molecular distance") + theme_classic() 

ggsave(filename = "just_BLCA_and_BRCA_and_THCA.png",plot = freq.plot.subset,height = 5,width = 7)
...
# need to redo to have the number of each amino acid sequenced. 
...
library(ggplot2)
frequency.plot <- ggplot(data = subset(miyata.melt, Frequency>0), aes(x=Distance, y= McLachlan_rel_freq)) + geom_point(alpha=0.8) + geom_smooth(method="glm",method.args=list(family=gaussian(link="log")),se = F) + facet_wrap(~Tumor_type, scales = "free") + labs(y="Normalized frequency",x="Molecular distance") + theme_classic() 

ggsave(filename = "Normalized_freq_vs_distance_McLachlan_method.png",plot = frequency.plot,height = 10,width = 10)


# 
# 
# miyata.melt[which(miyata.melt$Tumor_type=="LUAD"),][which.max(miyata.melt$McLachlan_rel_freq[which(miyata.melt$Tumor_type=="LUAD")]),]
# translations[which(translations[,"AA_letter"] %in% c("C","W")),]
# table(combined_full_data$trinucs[which(combined_full_data$tumor_type == "LUAD" & ( (combined_full_data$Amino_acid_reference=="W" & combined_full_data$Amino_acid_alternative=="C")  | (combined_full_data$Amino_acid_reference=="C" & combined_full_data$Amino_acid_alternative=="W")))])
# 
# table(combined_full_data$Nucleotide_trinuc_context[which(combined_full_data$tumor_type == "LUAD" & ( (combined_full_data$Amino_acid_reference=="W" & combined_full_data$Amino_acid_alternative=="C")  | (combined_full_data$Amino_acid_reference=="C" & combined_full_data$Amino_acid_alternative=="W")))])
# 
# table(combined_full_data$Amino_acid_codon[which(combined_full_data$tumor_type == "LUAD" & ( (combined_full_data$Amino_acid_reference=="W" & combined_full_data$Amino_acid_alternative=="C")  | (combined_full_data$Amino_acid_reference=="C" & combined_full_data$Amino_acid_alternative=="W")))])
# 
# 
# nrow(combined_full_data[which(combined_full_data$tumor_type == "LUAD" & (  (combined_full_data$Amino_acid_reference=="W" & combined_full_data$Amino_acid_alternative=="C"))),])
# 
# 
# this.tumor <- "GBM"
# # this.AA1 <- "W"
# # this.AA2 <- "R"
# miyata.melt[which(miyata.melt$Tumor_type==this.tumor),][which.max(miyata.melt$McLachlan_rel_freq[which(miyata.melt$Tumor_type==this.tumor)]),]
# 
# this.AA1 <- "W"
# this.AA2 <- "R"
# translations[which(translations[,"AA_letter"] %in% c(this.AA1,this.AA2)),]
# # miyata.melt[which(miyata.melt$Tumor_type==this.tumor),][which.max(miyata.melt$McLachlan_rel_freq[which(miyata.melt$Tumor_type==this.tumor)]),]
# table(combined_full_data$trinucs[which(combined_full_data$tumor_type == this.tumor & ( (combined_full_data$Amino_acid_reference==this.AA1 & combined_full_data$Amino_acid_alternative==this.AA2)  | (combined_full_data$Amino_acid_reference==this.AA2 & combined_full_data$Amino_acid_alternative==this.AA1)))])
# 
# table(combined_full_data$Nucleotide_trinuc_context[which(combined_full_data$tumor_type == this.tumor & ( (combined_full_data$Amino_acid_reference==this.AA1 & combined_full_data$Amino_acid_alternative==this.AA2)  | (combined_full_data$Amino_acid_reference==this.AA2 & combined_full_data$Amino_acid_alternative==this.AA1)))])
# 
# table(combined_full_data$Amino_acid_codon[which(combined_full_data$tumor_type == this.tumor & ( (combined_full_data$Amino_acid_reference==this.AA1 & combined_full_data$Amino_acid_alternative==this.AA2)  | (combined_full_data$Amino_acid_reference==this.AA2 & combined_full_data$Amino_acid_alternative==this.AA1)))])
# 
# combined_all_data[which(combined_all_data$tumor_type==this.tumor & ( (combined_all_data$AA_Ref==this.AA1 & combined_all_data$AA_Change==this.AA2)  | (combined_all_data$AA_Ref==this.AA2 & combined_all_data$AA_Change==this.AA1))),][order(combined_all_data$freq[which(combined_all_data$tumor_type==this.tumor & ( (combined_all_data$AA_Ref==this.AA1 & combined_all_data$AA_Change==this.AA2)  | (combined_all_data$AA_Ref==this.AA2 & combined_all_data$AA_Change==this.AA1)))],decreasing = T),]
# 
# 
# 
# this.tumor <- "LUSC"
...
# 
# 
# nrow(combined_all_data[which(combined_all_data$tumor_type=="LUAD" & combined_all_data$AA_Ref=="C" & combined_all_data$AA_Change=="W" & combined_all_data$freq>0),])
# nrow(combined_all_data[which(combined_all_data$tumor_type=="LUAD" & combined_all_data$AA_Change=="C" & combined_all_data$AA_Ref=="W" & combined_all_data$freq>0),])
# 
# 
# View(combined_all_data[which(combined_all_data$tumor_type==miyata.melt$Tumor_type[i] & 
#                                combined_all_data$AA_Ref== translations$AA_letter[which(translations$AA_short== miyata.melt$AA_Change[i])[1]] & 
...
# 
# 
# miyata.melt[which(miyata.melt$Tumor_type=="LUAD" & miyata.melt$AA_Ref %in% c("CYS","TRP") &  miyata.melt$AA_Change %in% c("CYS","TRP") ),]
# 
# 
# tally_array[,,"LUAD",1]
# 
# density.plot <- ggplot(data=subset(combined_all_data,freq>1),aes(x=gamma_epistasis)) + geom_density()  + theme_classic()  + scale_x_log10()
# 
# density.plot <- ggplot(data=subset(combined_all_data,freq>1),aes(x=gamma_epistasis)) + geom_area(stat="bin")  + theme_classic()  + scale_x_log10()
# 
# 
# density.plot <- ggplot(data=subset(combined_all_data,freq>1),aes(x=gamma_epistasis)) + geom_dotplot(stackdir = "center",dotsize = .75,stackratio=.75)  + theme_classic() + coord_flip() + coord_cartesian(ylim = c(-10000,10000))
# 
# 
# density.plot <- ggplot(data=subset(combined_all_data,freq>1),aes(x=gamma_epistasis)) + geom_dotplot(method="histodot",binwidth = 10,stackdir = "center",dotsize = 50)  + theme_classic()  
# 
# density.plot <- ggplot(data=subset(combined_all_data,freq>1),aes(x=gamma_epistasis)) + geom_dotplot(stackdir = "center",dotsize = 1)  + theme_classic() 
# 
# density.plot <- ggplot(data=subset(combined_all_data,freq>1),aes(x=gamma_epistasis)) + geom_dotplot(stackdir = "center",dotsize = .75,stackratio=.75)  + theme_classic()  +facet_wrap(~tumor_type, scales = "free")  + scale_y_continuous(NULL, breaks = NULL) + coord_flip()
# 
# density.plot <- ggplot(data=subset(combined_all_data,freq>1),aes(x=gamma_epistasis)) + geom_dotplot(stackdir = "center",dotsize = .75)  + theme_classic()  +facet_wrap(~tumor_type, scales = "free")  + scale_y_continuous(NULL, breaks = NULL) + coord_flip()
# 
# density.plot <- ggplot(data=subset(combined_all_data,freq>1),aes(x=gamma_epistasis)) + geom_density()  + theme_classic()  +facet_wrap(~tumor_type, scales = "free")  + scale_y_continuous(NULL, breaks = NULL) 
# 
# density.plot.facet <- ggplot(data=subset(combined_all_data,freq>1),aes(x=gamma_epistasis)) + geom_density() + facet_wrap(~tumor_type)  + theme_classic() + scale_x_log10() 
# 
# 
# density.plot.jitter <- ggplot(data=subset(combined_all_data,freq>1),aes(y=gamma_epistasis,x=1)) + geom_jitter(alpha=0.5,height = 0,width = 0.2)  + theme_classic()  +facet_wrap(~tumor_type, scales = "free_y") + coord_cartesian(xlim=c(.6,1.4))
# ggsave(filename = "gamma_plot_jitter.png",plot = density.plot.jitter,height = 10,width = 10)
# 
# ggsave(filename = "gamma_density_all.png",plot = density.plot,height = 10,width = 10)
# ggsave(filename = "gamma_density_facet.png",plot = density.plot.facet,height = 10,width = 10)
# 
# 
# 
# 
...
# }
# 
# trinucs.split <- strsplit(x = combined_full_data.AA$Nucleotide_trinuc_context,split = "")
# 
# trinucs.split.mat <- matrix(data = unlist(trinucs.split),ncol=3,byrow = T,nrow=length(trinucs.split))
# 
# for(i in 1:nrow(combined_full_data.AA)){
#   # orig.trinuc.split <- strsplit(x = combined_full_data.AA$Nucleotide_trinuc_context[i],split = "")
#   if(trinucs.split[[i]][2] %in% c("C","T")){
#     combined_full_data.AA$this_trinuc[i] <-  combined_full_data.AA$Nucleotide_trinuc_context[i]
#     combined_full_data.AA[i,c("trinuc_upstream","trinuc_center","trinuc_downstream")] <- trinucs.split[[i]]
#   }else{
#     
#   }
#   
#   if((i %% 10000)==0){print(round(i/nrow(combined_full_data.AA),3))}
# }
# 
# 
# 
...
#                           mutant = strsplit(trinuc.mutation.df[this.position.list[position],"codon_2"],"")[[1]][2])
#         
#         
#       }
#       if(position>=7){
...
# 
# library(ggplot2)
# frequency.plot_trinuc <- ggplot(data = subset(miyata.melt, Frequency>0), aes(x=Distance, y= McLachlan_rel_freq_prop/trinuc_rate_prop)) + geom_point(alpha=0.8) + geom_smooth(method="glm",method.args=list(family=gaussian(link="log")),se = F) + facet_wrap(~Tumor_type, scales = "free") + labs(y="Normalized frequency",x="Molecular distance") + theme_classic() 
# 
# ggsave(filename = "normalized_freq_vs_distace_trinuc.png",plot = frequency.plot_trinuc,height = 10,width = 10)
# 
# frequency.plot_trinuc_noCYSTRP <- ggplot(data = subset(miyata.melt, Frequency>0 & (AA_Ref!="CYS"&AA_Change!="TRP")), aes(x=Distance, y= McLachlan_rel_freq_prop/trinuc_rate_prop)) + geom_point(alpha=0.8) + geom_smooth(method="glm",method.args=list(family=gaussian(link="log")),se = F) + facet_wrap(~Tumor_type, scales = "free") + labs(y="Normalized frequency",x="Molecular distance") + theme_classic()
# 
# ggsave(filename = "normalized_freq_vs_distace_trinuc_noCYSTRP.png",plot = frequency.plot_trinuc_noCYSTRP,height = 10,width = 10)
# 
# miyata.melt$prop_w_trinuc <- miyata.melt$McLachlan_rel_freq_prop/miyata.melt$trinuc_rate_prop
# 
# 
# this.tumor <- "LUAD"
# # this.AA1 <- "W"
# # this.AA2 <- "R"
# miyata.melt[which(miyata.melt$Tumor_type==this.tumor),][which.max(miyata.melt$prop_w_trinuc[which(miyata.melt$Tumor_type==this.tumor)]),]
# 
# this.AA1 <- "W"
# this.AA2 <- "C"
# translations[which(translations[,"AA_letter"] %in% c(this.AA1,this.AA2)),]
...
# 
# 
# miyata.luad <- miyata.melt[which(miyata.melt$Tumor_type=="LUAD"),]
# miyata.luad.TRP.CYS <- miyata.luad[which((miyata.luad$AA_Ref=="CYS" & miyata.luad$AA_Change=="TRP") | (miyata.luad$AA_Ref=="TRP" & miyata.luad$AA_Change=="CYS")),]
# translations[which(translations[,"AA_letter"] %in% c(this.AA1,this.AA2)),]
# # 
# 
# 
...
# 
# library(ggplot2)
# frequency.plot_trinuc_scaled <- ggplot(data = subset(miyata.melt, Frequency>0), aes(x=Distance, y= trinuc_normalized)) + geom_point(alpha=0.8,size=.5) + geom_smooth(method="glm",method.args=list(family=gaussian(link="log")),se = F) + facet_wrap(~Tumor_type, scales = "free", ncol=4) + labs(y="Normalized frequency",x="Molecular distance") + theme_classic() + coord_cartesian(ylim=c(0,8))
# 
# ggsave(filename = "normalized_freq_vs_distace_trinuc_scaled.png",plot = frequency.plot_trinuc_scaled,height = 8,width = 6)
